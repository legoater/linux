...
static int mw_spi_set_cfg(struct mw_spi *mspi, const struct spi_mem_op *op)
{
	...
	/* Preserve clock settings (from FW/HW) */
...
static int mw_spi_setup(struct spi_device *spi)
{
	...
	dev_dbg(&mspi->pdev->dev, "Requesting %d MHz SPI frequency\n",
	...
	clk_div = (mspi_read(mspi, MSPI_CFG) & MSPI_CFG_CKDIV_MASK)
		>> MSPI_CFG_CKDIV_SHIFT;
	spi_clk_rate = clk_rate / ((clk_div + 1) * 2);

	/* TODO: set new clock rate */

	dev_info(&mspi->pdev->dev, "Using SPI frequency %ld MHz\n",
		 spi_clk_rate/ 1000000);
	return 0;
}
...
static int mw_spi_probe(struct platform_device *pdev)
{
	...
	mspi->pclk = devm_clk_get(&pdev->dev, "clocks");
	if (IS_ERR(mspi->pclk))
...
{
	struct mw_spi *mspi = platform_get_drvdata(pdev);
